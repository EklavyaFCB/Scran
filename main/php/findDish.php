...
//php.get
 ...
 //    // $cuisineSize = sizeof($cuisine);
    ...
    // $query = "SELECT dishID FROM Dish WHERE course = 'Starter' AND cuisine = 'British' AND diet = 'Vegetarian' ORDER BY RAND() LIMIT 1;";
    ...
    if ($course != "" && $cuisine == "") {
    	$query = "SELECT dishID FROM Dish WHERE course = '$course' ORDER BY RAND() LIMIT 1;";
    } else if ($course == "" && $cuisine != "") {
        $query = "SELECT dishID FROM Dish WHERE cuisine = '$cuisine' ORDER BY RAND() LIMIT 1;";
    } else if ($diet == "" && $req != "") {
